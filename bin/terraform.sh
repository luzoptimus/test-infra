...
ARGS=$(getopt \
         ...
         -l "help,version,bootstrap,action:,bucket-prefix:,build-id:,component:,environment:,group:,project:,region:,target:,lockfile:,detailed-exitcode,no-color,compact-warnings" \
         -n "${0}" \
         -- \
         "$@");
...
while true; do
  case "${1}" in
    ...
    --bootstrap)
      shift;
      echo '[ERROR] Bootstrapping is not enabled on this project' >&2;
      exit 1;
      bootstrap="true";
      ;;
    -d|--detailed-exitcode)
...
# Set DynamoDB table for state locks. Set default to match S3 bucket naming convention.
readonly dynamodb="${project}-tfscaffold-${aws_account_id}-${region}";
echo -e "Using DynamoDB table ${dynamodb} for state locking";
...
# Build up the target arguments for terraform command line "${target[@]}"
declare -a tf_var_targets;
if [[ "${target[@]}" != "null" ]]; then
  for target_mod in "${target[@]}"; do
    tf_var_targets+=" -target=${target_mod}";
  done;
fi;
...
# Validate terraform version and assign the correct backend_config 
if [ $(terraform version | head -n1 | cut -d" " -f2 | cut -d"." -f1) == "v1" ] && [ $(terraform version | head -n1 | cut -d" " -f2 | cut -d"." -f2) -lt 10 ]; then
    # For versions prior to 1.10 (use dynamodb_table)
 ...
 encrypt = true
    ...
    # For versions 1.10 or higher (use use_lockfile)
    readonly backend_config="terraform {
 backend \"s3\" {
 region = \"${region}\"
 bucket = \"${bucket}\"
