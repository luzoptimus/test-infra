    ...
    "Statement": [
        {
            ...
            "Resource": "arn:aws:s3:::${format("%s-%s-s3-cloudtrail-%s", var.subproject, var.environment, var.project)}"
        },
        {
            ...
            "Action": "s3:PutObject",
            "Resource": "arn:aws:s3:::${format("%s-%s-s3-cloudtrail-%s", var.subproject, var.environment, var.project)}/AWSLogs/${var.aws_account_id}/*",
            "Condition": {
                "StringEquals": {
...
resource "aws_cloudwatch_log_group" "backuping_cloudwatch_log_group" {
  ...
  name = format("%s-%s-cwlogs-cloudtrail-%s", var.subproject, var.environment, var.project)
  tags =  var.tags
  }
...
resource "aws_iam_policy" "backuping_cloudtrail_cloudwatch_policy" {
  count = var.enable_cw_logs && var.region == "us-east-1" ? 1 : 0
  name        = format("%s-%s-poli-cwcloudtrail-%s", var.subproject, var.environment, var.project)
...
resource "aws_iam_role" "backuping_cloudtrail_cloudwatch_role" {
  count = var.enable_cw_logs && var.region == "us-east-1" ? 1 : 0
  name = format("%s-%s-role-cwcloudtrail-%s", var.subproject, var.environment, var.project)
  path = "/service-role/"
...
resource "aws_s3_bucket" "backuping_logs_bucket" {
  count = var.enable_cw_logs && var.region == "us-east-1" ? 1 : 0
  bucket = format("%s-%s-s3-cloudtrail-%s", var.subproject, var.environment, var.project)
  force_destroy = true
...
resource "aws_cloudtrail" "backuping_cloudtrail" {
  count = var.enable_cw_logs && var.region == "us-east-1" ? 1 : 0
  name = format("%s-%s-cwlogs-cloudtrail-%s", var.subproject, var.environment, var.project) ##"backuping-${var.env}-cloudtrail"
