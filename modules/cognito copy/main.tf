...
resource "aws_cognito_user_pool" "user_pool" {
  ...
  name                     = format("%s-%s-%s-%s", var.namespace, var.environment, var.project, var.subproject)
  ...
  # Configuracion para AdminCreateUser
  ...
  dynamic "lambda_config" {
    for_each = try(coalesce(
      ...
      var.lambda_kms_key_id,
      var.lambda_arn_email,
      var.lambda_version_email
    ), null) == null ? [] : [true]
    ...
    content {
      ...
      custom_email_sender  {
        lambda_arn  = var.lambda_arn_email
        lambda_version = var.lambda_version_email
      }
      # custom_sms_sender  {        
      #   lambda_arn  = var.lambda_arn_sms
      #   lambda_version = var.lambda_version_sms
      # }         
    }
  }
...
resource "aws_cognito_user_pool_ui_customization" "example" {
  count = var.module_enabled && var.domain != null && var.image_file != null ? 1 : 0
  css        = var.custom_css
  image_file = filebase64(var.image_file)

  # Refer to the aws_cognito_user_pool_domain resource's
  # user_pool_id attribute to ensure it is in an 'Active' state
  user_pool_id = aws_cognito_user_pool_domain.domain[0].user_pool_id
}
...
resource "aws_cognito_user" "user" {
  ...
  attributes = merge({
    email                    = var.user_name[count.index]
    email_verified           = true
   }, var.attributes_user)

  # dynamic "attributes" {
  #   for_each = var.attributes_user
  #   iterator = attribute

  #   content {
  #     email                    = try(attribute.value.email, var.user_name) 
  #     email_verified           = try(attribute.value.email_verified, true)
  #   }
  # }
...
resource "aws_cognito_user_group" "main" {
  ...
  user_pool_id = aws_cognito_user_pool.user_pool[0].id
  description  = format("Managed by Terraform. Group %s", var.groups[count.index] )
