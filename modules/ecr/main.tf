...
resource "aws_ecr_repository" "name" {
  ...
  tags = merge({ "Name" = format("%s/%s/%s", var.project, var.subproject, each.value) }, { "Project" = var.project }, var.tags)
}
...
data "aws_iam_policy_document" "resource_full_access" {
  ...
  statement {
    ...
    principals {
      ...
      identifiers = var.service_full_access
    }
